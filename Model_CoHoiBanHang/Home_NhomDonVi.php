        ...
        <div class="content">
            <div class="button-container">
                ...
                <button class="button" onclick="openModal()">
                    <i class="fas fa-plus"></i> Thêm Nhóm Đơn vị
                </button>
            ...
            <table id="group-table">
                ...
                <tbody>
                    ...
                    // Truy vấn dữ liệu từ bảng `nhomdonvi`
                    $sql = "SELECT Id_NhomDonVi, TenDonVi, MoTa FROM nhomdonvi";
                    ...
                    if ($result->num_rows > 0) {
                        ...
                        while ($row = $result->fetch_assoc()) {
                            ...
                            echo "<td>";
                            echo "<button class='button edit-button' data-id='" . $row['Id_NhomDonVi'] . "'>Chỉnh Sửa</button>";
                            echo " <button class='button delete-button' data-id='" . $row['Id_NhomDonVi'] . "'>Xóa</button>";
                            echo "</td>";
                            echo "</tr>";
                            $stt++;
                        }
                    } else {
                        echo "<tr><td colspan='4'>Không có đơn vị nào.</td></tr>";
                    }
            ...
            <div id="myModal" class="modal">
                <div class="modal-content">
                    ...
                    <h2>Thêm Mới Nhóm Đơn Vị</h2>
                    <form id="addGroupForm" action="add_group.php" method="POST">
                        <div class="form-group">
                            <label for="tenDonVi" class="form-label">Tên Đơn Vị:</label>
                            <input type="text" id="tenDonVi" name="tenDonVi" class="form-control" placeholder="Nhập tên đơn vị" required>
                        </div>
                        <div class="form-group">
                            <label for="moTa" class="form-label">Mô Tả:</label>
                            <textarea id="moTa" name="moTa" class="form-control" placeholder="Nhập mô tả" required></textarea>
                        </div>
                        <div class="modal-footer">
                            <button type="submit" class="button save-button">Lưu</button>
                            <button type="button" class="button cancel-button" onclick="closeModal()">Hủy</button>
                        </div>
                    </form>
                </div>
            </div>
                    ...
                    fetch('add_nhomdonvi.php', {
                        ...
                        .then(data => {
                            if (data.status === 'success') {
                                ...
                                window.location.href = 'Home_NhomDonVi.php';
                            } else {
                ...
                document.addEventListener('DOMContentLoaded', function() {
                    ...
                    deleteButtons.forEach(button => {
                        button.addEventListener('click', function() {
                            const idNhomDonVi = this.getAttribute('data-id');
                            if (confirm('Bạn có chắc chắn muốn xóa nhóm đơn vị này?')) {
                                fetch('delete_nhomdonvi.php', {
                                        method: 'POST',
                                        headers: {
                                    ...
                                    .then(data => {
                                        if (data.status === 'success') {
                                            alert(data.message);
                                            window.location.href = 'Home_NhomDonVi.php'; // Chuyển hướng về trang Home_NhomDonVi.php
                                        } else {
                                            alert(data.message);
                                        }
                                    })
                    ...
                    editButtons.forEach(button => {
                        button.addEventListener('click', function() {
                            const idNhomDonVi = this.getAttribute('data-id');
                            window.location.href = 'edit_nhomdonvi_form.php?id=' + idNhomDonVi;
                        });
                    });
                });
            </script>
